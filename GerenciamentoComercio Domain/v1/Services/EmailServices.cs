using Catalde.Tools.Email;
using GerenciamentoComercio_Domain.Utils.EmailSender;
using GerenciamentoComercio_Domain.Utils.EmailSender.EmailConfig;
...
namespace GerenciamentoComercio_Domain.v1.Services
{
    public class EmailServices : IEmailServices
    {
        ...
        {
            Correio.EnviaEmailAnexo("no-reply@centraldefretes.com", user.Email,
                "Plataforma de cotação - Recuperação de senha", Message("Recuperação de senha",
                user.Email, Security.EncryptString(user.Id.ToString())), "smtp.office365.com", "587",
                "no-reply@centraldefretes.com", "senha#123", string.Empty, true);


            //string emailContent = Message("Gerenciamento de comércio - Recuperação de senha",
            //    user.FullName, Security.EncryptString(user.Id.ToString()));

            //_emailSender.Send(user.Email, "Gerenciamento de comércio - Recuperação de senha", emailContent);
        }
        ...
        private static string Message(string title, string user, string token)
        {
            ...
            sMensagem += "<img alt=\"\" src=\"http://cotacao.centraldefretes.com/img/logo.png\" width=\"430px\" />";
            ...
            sMensagem += $"<a href='http://gerenciamentocomercio.victorsavoi.dev.br/update-password/{token}'>Link para recuperação de senha.</a>";
